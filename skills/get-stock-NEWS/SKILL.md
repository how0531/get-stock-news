...
| 工商時報 ctee | 列表頁 HTML，publish_ts 取自 URL 內嵌日期（`scripts/ctee.py`） | 列表頁更新頻率 | ✅ sitemap 回補（`backfill_ctee.py`） | 無公開 RSS、WP API 已關閉(404)；URL `/news/{YYYYMMDD}...` 含日期，`fetch_content=True` 可抓全文與 `<meta pubdate>` 精確時間 |
...
**統一記錄格式（Phase 3）**：所有來源清洗後對齊 `data/site_maps/_schema.json` 定義的 15 欄位，
分四段：①識別（source/source_label/category）②內容（title/**author**/summary/**content**/url）
③時間 PIT（publish_ts/ingestion_ts/actionable_ts）④衍生（tickers/tags/**also_reported_by**，日終/下游補）。

- **author 作者**：有就帶、缺則空字串。RSS 取 `dc:creator`/`author`（`common.extract_author_from_entry`）；
  工商時報 `fetch_content=True` 進內頁取 byline；官方公告填發布公司名。
- **content 全文**：udn / ctee / rss 三個收集器統一以 `fetch_content=True` 逐篇進內頁抽取
  （多一個 request/篇，禮貌性 sleep）；cnyes 列表 API 已內含全文；官方公告「說明」即全文（公開法定資料）。
  - **日終彙整 `main.py` 預設開啟全文抓取**（批次跑一次、延遲不敏感），`--no-content` 可關閉。
  - **盤中 `watch_intraday` 預設也補抓全文**，但只對通過去重的「新項目」抓（`enrich_content`），
    避免每輪對同一批列表重複抓內頁被擋；`--no-content` 可關閉以維持最低延遲。
  內文容器選擇器已逐家登記於 `SOURCES[sid]['body_selectors']`（依信心排序、結尾留 `article`/`main` 泛用 fallback）：
  中時 `div.article-body`、Yahoo `div.caas-body`（高信心）；ETtoday `div.story`、中央社 `div.paragraph`、
  自由 `div.text`、科技新報 `div.indent`（中信心）；MoneyDJ 版型未定靠 fallback（低信心）。未登記者用 `common.DEFAULT_BODY_SELECTORS`。待連線實測校正。
...
| Script | Purpose |
|--------|---------|
| `scripts/process_day.py` | 當日 raw + stream → processed Parquet（PIT 三時間戳、兩層去重：標題完全相同 + 內文指紋轉載） |
